...
def source_to_reference_mappings(
    ...
    reference_heavy = Chem.RemoveHs(reference, sanitize=False)

    source_count = source_heavy.GetNumAtoms()
    reference_count = reference_heavy.GetNumAtoms()

    if source_count != reference_count:
        raise RuntimeError(
            "Source and reference ligands have different heavy-atom "
            f"counts: {source_count} versus {reference_count}"
        )

    # First attempt the original strict graph match.
    ...
    if mappings:
        print(
            f"Strict source/reference mappings found: "
            f"{len(mappings)}"
        )
        return list(mappings)

    print(
        "Strict ligand mapping failed; trying an element-preserving "
        "MCS that ignores bond-order and formal-charge differences."
    )

    mcs = rdFMCS.FindMCS(
        [source_heavy, reference_heavy],
    ...
    if mcs.canceled:
        raise RuntimeError(
            "Source/reference ligand MCS search timed out."
        )

    print(
        f"MCS coverage: {mcs.numAtoms}/{source_count} heavy atoms; "
        f"{mcs.numBonds} bonds"
    )

    if mcs.numAtoms != source_count:
        raise RuntimeError(
            "Could not map the complete ligand. "
            f"MCS covered {mcs.numAtoms} of {source_count} heavy atoms."
        )
    ...
    if query is None:
        raise RuntimeError(
            "Could not construct an RDKit query from the ligand MCS."
        )
    ...
    if not source_matches or not reference_matches:
        raise RuntimeError(
            "The full ligand MCS was found, but atom mappings could "
            "not be enumerated."
        )

    complete_mappings: set[tuple[int, ...]] = set()

    for source_match in source_matches:
        for reference_match in reference_matches:
            mapping: list[int | None] = [None] * source_count

            for query_index, source_index in enumerate(source_match):
                mapping[source_index] = reference_match[query_index]

            if any(index is None for index in mapping):
                continue

            complete_mappings.add(
                tuple(int(index) for index in mapping)
            )

    if not complete_mappings:
        raise RuntimeError(
            "No complete source-to-reference ligand mappings were "
            "generated from the MCS."
        )

    print(
        f"Complete MCS mappings generated: "
        f"{len(complete_mappings)}"
    )

    return sorted(complete_mappings)
